																		...
																		property="principal.realUser.empImg" />">
																			...
																			<div class="mx-auto">
																				<input class="form-check-input invisible"
																					id="isMody" type="checkbox" value="" checked />
																			</div>
																		</div>
																	</div>
																</div>
	...
	$(document).ready(function() {
		...
		$("#getApproval").on('click', function() {
			...
			vacRadio.forEach(function(item) {
				  ...
				  var vacGrnt = item.getAttribute('vac-grnt-no')
				  item.setAttribute("code-name", codeNm);
				  item.setAttribute("remain-vac", remain);
				  item.setAttribute("vac-grnt-no", vacGrnt);
			})
...
// 	휴가 종류 선택 모달에서 확인 버튼 클릭
	...
	// 기안 의견 데이터 전달
		...
		$("#setOpnionBtn").on("click", function() {
			...
			var opinionTemp = `<input class="form-control-plaintext" name="draftOpnVOList[` + count + `].opnnCn" readonly value="${draftrealUser.empNm}: ` + opinion +  `"/>`
			opinionTemp += `<input type="hidden" name="draftOpnVOList[` + count + `].empCd" value="${empVO.empCd}"/>`
			...
			//[{"empCd":"NAVER_2023060009","empNm":"김윤아","userTelno":"01029949383","userEml":"2023060009@naver.com","empImg":"NULL","deptNo":1,"jbgdCd":"JG001","jbtlCd":"JT001"}
		 	...
		 	nodes.forEach(node => {
			  ...
			  // idCard를 가져올 때 index가 아니라 id를 가져와야 함!!
		 	  parents[node.parent].push({name : node.child, id : node.id, jbgdCd : node.jbgdCd, empImg: node.img, deptNm : node.dept});
		 	});
